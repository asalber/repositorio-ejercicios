...
## DATA GENERATION
require(xtable)
clases <- c("(70,80]", "(80, 90]", "(90, 100]", "(100, 110]", "(110, 120]", "(120, 130]", "(130, 140]", "(140, 150]")
nx <- c(2, 5, 14, 12, 8, 5, 3, 1)
ny <- c(3, 9, 14, 13, 5, 3, 2, 1)
...
\begin{question}
\puntos{4.5}Se ha medido la tensión arterial sistólica (en mmHg) en dos grupos de 50 personas cada uno de dos poblaciones $A$ y $B$, obteniendo los siguientes resultados:
...
Se pide: 
\begin{enumerate}
\item ¿En cuál de los dos grupos hay una mayor dispersión relativa con respecto a la media de la tensión sistólica?
\item ¿Por debajo de qué tensión sistólica se encuentra el 40\% de las personas del grupo de la población $B$?
\item ¿Cuál de las dos distribuciones de la tensión sistólica es más asimétrica? ¿Cuál es menos apuntada? ¿Pueden provenir estas muestras de poblaciones normales?
\item ¿Qué tensión sistólica es relativamente más alta, 122 mmHg en el grupo de la población $A$, o 120 mmHg en el grupo de la población $B$?
\item Si a las 50 personas de la población $B$ se les mide la tensión sistólica con otro tensiómetro, y la tensión obtenida ($Y$) está relacionada con la del primer tensiómetro ($X$) mediante la ecuación $y=1.05x-1.2$, ¿en cuál de las dos tensiones $X$ o $Y$ es más representativa la media? Justificar la respuesta.
\end{enumerate}
