...
\begin{question}
\puntos{5 } Un estudio intenta poner a punto una nueva técnica de detección de un cierto anticuerpo. Para ello se utiliza un inmunosensor piezoeléctrico, que permite medir el cambio en la señal en Hz al variar la concentracióndel anticuerpo ($\mu$g/ml). Se recolectaron los siguientes datos:
$$
